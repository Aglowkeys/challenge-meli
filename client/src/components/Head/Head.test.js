    ...
    it('Renderiza meta tags', async () => {
        ...
        await waitFor(() => {
            expect(getMetaTag('description')).toEqual(
                'Desafío técnico de Front End enviado por Mercado Libre.'
            );
            expect(getMetaTag('author')).toEqual('Emiliano Alfonso');
            expect(getMetaTag('author')).not.toEqual('Emiliano Alfonsoasd');
        });
    });
});
